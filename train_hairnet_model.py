...
def parse_args():
    ...
    parser.add_argument('--weights', type=str, default='yolov8n.pt',
                        help='初始权重，默认为yolov8n.pt')
    parser.add_argument('--device', type=str, default='',
                        help='训练设备，例如cuda:0或cpu')
    parser.add_argument('--name', type=str, default='hairnet_model',
                        help='实验名称，默认为hairnet_model')
...
def train_yolov8(args):
    ...
    if os.path.exists(best_model_path):
        ...
        # 复制最佳模型到models目录
        final_model_path = os.path.join('models', 'hairnet_detection.pt')
...
def main():
    ...
    try:
        ...
        if model_path:
            ...
            print("\n使用以下命令测试模型:")
            print(f"python test_hairnet_model.py --weights {model_path} --source path/to/test/image.jpg")
            return 0
        else:
            print("\n模型训练失败")
            return 1
