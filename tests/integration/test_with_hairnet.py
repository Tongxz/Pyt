        ...
        if response.status_code == 200:
            ...
            for i, detection in enumerate(detections.get('detections', [])):
                ...
                if 'debug_info' in detection:
                    ...
                    print(f"    检测条件: 浅蓝={debug.get('has_light_blue_hairnet', False)}, 一般={debug.get('has_general_hairnet', False)}, 浅色={debug.get('has_light_hairnet', False)}, 基础={debug.get('has_basic_hairnet', False)}")
                    if 'error' in debug:
