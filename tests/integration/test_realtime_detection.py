...
def create_realistic_person_image(with_hairnet=False):
    ...
    draw.ellipse(
        [
            head_center[0] - head_radius + 5,
            head_center[1] - head_radius + 5,
            head_center[0] + head_radius - 5,
            head_center[1] - head_radius + 30,
        ],
        fill="brown",
        outline="black",
        width=1,
    ...
    if with_hairnet:
        ...
        # 水平线
        for y in range(
            head_center[1] - head_radius + 10, head_center[1] - head_radius + 35, 8
        ):
            draw.line(
                [
        ...
        # 垂直线
        for x in range(
            head_center[0] - head_radius + 10, head_center[0] + head_radius - 10, 8
        ):
            draw.line(
                [
                    x,
                    head_center[1] - head_radius + 10,
                    x,
                    head_center[1] - head_radius + 35,
                ],
                fill=net_color,
                width=2,
            )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, person in enumerate(persons, 1):
                print(
                    f"  人员{i}: 发网={person.get('has_hairnet', False)}, 置信度={person.get('confidence', 0.0):.3f}"
                )
