    ...
    try:
        ...
        for image_path in test_images:
            ...
            try:
                ...
                # 计算发网相关统计
                compliant_count = len([r for r in result.hairnet_results if r.get('has_hairnet', False)])
                print(f"    佩戴发网人数: {compliant_count}")
                total_persons = len(result.person_detections)
                compliance_rate = compliant_count / total_persons if total_persons > 0 else 0
                ...
                if result.hairnet_results:
                    ...
                    for hairnet_result in result.hairnet_results:
                        ...
                        print(f"    人员{person_id}: 佩戴发网={has_hairnet}, 置信度={confidence:.3f}")
