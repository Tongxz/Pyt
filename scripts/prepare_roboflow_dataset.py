...
def parse_args():
    ...
    parser.add_argument(
        "--input", type=str, required=True, help="Roboflow数据集路径（ZIP文件或解压后的目录）"
    )
    parser.add_argument(
        "--output", type=str, default="./datasets/hairnet", help="输出数据集目录"
    )
    parser.add_argument(
        "--format",
        type=str,
    ...
    parser.add_argument("--extract-only", action="store_true", help="仅解压数据集，不进行处理")
...
def main():
    ...
    try:
        ...
        logger.info("\n训练命令示例:")
        logger.info(
            f"python scripts/train_hairnet_model.py --data {dataset_path}/data.yaml --epochs 100 --batch-size 16 --img-size 640 --model yolov8n.pt --name hairnet_model"
        )
