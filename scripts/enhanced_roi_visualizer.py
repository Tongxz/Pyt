...
class EnhancedROIVisualizer:
    ...
    def analyze_and_visualize_enhanced(self, image: np.ndarray) -> Optional[Figure]:
        ...
        for person_idx, detection in enumerate(detections):
            ...
            # 进行发网检测
            hairnet_result = self.hairnet_detector._detect_hairnet_with_pytorch(head_roi)
            ...
            ax1.imshow(image)
            ax1.set_title(f'Person {person_idx + 1}\n人体检测 (conf: {confidence:.2f})', fontsize=10)
            ...
            ax4.set_title(f'颜色分析\n蓝色: {blue_ratio:.3f}\n白色: {white_ratio:.3f}\n总计: {total_ratio:.3f}', fontsize=10)
            ax4.axis('off')
            ...
            ax5.set_title(f'发网检测结果\n{status}\n置信度: {hairnet_confidence:.3f}\n颜色: {hairnet_color}', 
                         fontsize=10, color=color)
            ax5.axis('off')
        
        # 添加总体标题
        fig.suptitle('Enhanced Hairnet Detection ROI Analysis Results', fontsize=16, y=0.98)
