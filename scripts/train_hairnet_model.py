...
def parse_args():
    ...
    parser.add_argument('--device', type=str, default='auto',
                        help='训练设备，可选：cpu, cuda, auto')
    parser.add_argument('--name', type=str, default='hairnet_model',
                        help='实验名称')
...
def create_data_yaml(data_path):
    ...
    data_yaml = {
        ...
        'test': str(test_dir) if test_dir.exists() else str(val_dir) if val_dir.exists() else str(train_dir),
        'nc': len(classes),
        'names': classes
    }
...
def train_model(args):
    ...
    if args.pretrained:
        ...
        model_yaml = f'yolov8{args.model[6] if args.model.startswith("yolov8") else "n"}.yaml'
