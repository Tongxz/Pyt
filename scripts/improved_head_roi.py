...
class ImprovedHeadROIExtractor:
    ...
    def __init__(self):
        ...
        try:
            ...
            for path in possible_paths:
                try:
                    if os.path.exists(path):
                        front_face_path = os.path.join(path, 'haarcascade_frontalface_default.xml')
                        profile_face_path = os.path.join(path, 'haarcascade_profileface.xml')
                    ...
                    try:
                        if os.path.exists(path):
                            ...
                            if os.path.exists(profile_face_path):
                                self.profile_cascade = cv2.CascadeClassifier(profile_face_path)
            ...
            self.face_detection_available = (self.face_cascade is not None and not self.face_cascade.empty())
    ...
    def detect_faces_in_region(self, image: np.ndarray, region_bbox: List[int]) -> List[Dict]:
        """在指定区域内检测面部"""
        ...
        gray_roi = cv2.cvtColor(roi_region, cv2.COLOR_RGB2GRAY) if len(roi_region.shape) == 3 else roi_region
    ...
    def extract_head_roi_with_keypoints(self, image: np.ndarray, bbox: List[float], keypoints: Optional[List[float]] = None) -> Tuple[Optional[np.ndarray], Optional[Dict]]:
        """使用关键点提取头部ROI"""
        ...
        for name, (x_idx, y_idx, conf_idx) in keypoint_indices.items():
            if x_idx < len(keypoints) and y_idx < len(keypoints) and conf_idx < len(keypoints):
        ...
        # 根据检测到的关键点类型调整扩展策略
        if 'nose' in head_keypoints and ('left_eye' in head_keypoints or 'right_eye' in head_keypoints):
            # 有鼻子和眼睛，可以更精确地定位
    ...
    def extract_head_roi_with_face_detection(self, image: np.ndarray, bbox: List[float]) -> Tuple[Optional[np.ndarray], Optional[Dict]]:
        """使用面部检测提取头部ROI"""
    ...
    def extract_head_roi_proportional(self, image: np.ndarray, bbox: List[float]) -> Tuple[Optional[np.ndarray], Optional[Dict]]:
        """基于比例的头部ROI提取（改进版）"""
        x1, y1, x2, y2 = map(int, bbox)
    ...
    def extract_head_roi_multi_method(self, image: np.ndarray, bbox: List[float], keypoints: Optional[List[float]] = None) -> Tuple[Optional[np.ndarray], Optional[Dict]]:
        """多方法融合的头部ROI提取"""
        methods_tried = []
        best_roi = None
        best_info = None
        ...
        # 方法1: 关键点检测
        if keypoints:
            roi_kp, info_kp = self.extract_head_roi_with_keypoints(image, bbox, keypoints)
    ...
    def visualize_comparison(self, image_path: str, save_path: Optional[str] = None):
        ...
        for person_idx, detection in enumerate(detections):
            ...
            # 尝试不同的方法
            roi_kp, info_kp = self.extract_head_roi_with_keypoints(image_rgb, bbox, keypoints)
            roi_face, info_face = self.extract_head_roi_with_face_detection(image_rgb, bbox)
            roi_prop, info_prop = self.extract_head_roi_proportional(image_rgb, bbox)
            roi_multi, info_multi = self.extract_head_roi_multi_method(image_rgb, bbox, keypoints)
            ...
            if roi_kp is not None and roi_kp.size > 0:
                ...
                score = self.evaluate_roi_quality(roi_kp, info_kp)
                axes[0, 1].set_title(f'关键点方法\n质量评分: {score:.2f}\n关键点数: {info_kp.get("keypoints_count", 0)}')
            else:
                axes[0, 1].text(0.5, 0.5, '关键点方法失败', ha='center', va='center', transform=axes[0, 1].transAxes)
            ...
            if roi_face is not None and roi_face.size > 0:
                ...
                score = self.evaluate_roi_quality(roi_face, info_face)
                axes[0, 2].set_title(f'面部检测方法\n质量评分: {score:.2f}\n类型: {info_face.get("face_type", "unknown")}')
            else:
                axes[0, 2].text(0.5, 0.5, '面部检测失败', ha='center', va='center', transform=axes[0, 2].transAxes)
            ...
            if roi_prop is not None and roi_prop.size > 0:
                ...
                score = self.evaluate_roi_quality(roi_prop, info_prop)
                axes[1, 0].set_title(f'改进比例方法\n质量评分: {score:.2f}\n宽高比: {info_prop.get("aspect_ratio", 0):.2f}')
            else:
                axes[1, 0].text(0.5, 0.5, '比例方法失败', ha='center', va='center', transform=axes[1, 0].transAxes)
            ...
            if roi_multi is not None and roi_multi.size > 0:
                ...
                axes[1, 1].set_title(f'多方法融合 (最佳)\n质量评分: {score:.2f}\n选择方法: {best_method}')
            else:
                axes[1, 1].text(0.5, 0.5, '多方法融合失败', ha='center', va='center', transform=axes[1, 1].transAxes)
            ...
            if roi_multi is not None and roi_multi.size > 0:
                ...
                # 简单的发网检测逻辑
                mask_ratio = np.sum(light_blue_mask > 0) / (light_blue_mask.shape[0] * light_blue_mask.shape[1])
                wearing_hairnet = mask_ratio > 0.05  # 如果蓝色区域超过5%
                
                axes[1, 2].imshow(light_blue_mask, cmap='hot')
                axes[1, 2].set_title(f'发网检测结果\n佩戴发网: {"是" if wearing_hairnet else "否"}\n蓝色区域比例: {mask_ratio:.3f}')
            else:
                axes[1, 2].text(0.5, 0.5, '无法进行发网检测', ha='center', va='center', transform=axes[1, 2].transAxes)
            ...
            if save_path:
                person_save_path = save_path.replace('.png', f'_person_{person_idx + 1}.png')
