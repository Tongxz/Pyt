...
def fix_hairnet_detector_tests():
    ...
    replacements = [
        (
            "test_extract_head_region_keypoints_bbox",
            "test__extract_head_roi_from_bbox_bbox",
        ),
        (
            "test_extract_head_region_keypoints_keypoints",
        ...
        (
            "self.detector.extract_head_region_keypoints",
            "self.detector._extract_head_roi_from_bbox",
        ),
        ("self.detector.preprocess_image", "self.detector._preprocess_image"),
        (
            "self.pipeline._calculate_compliance_rate",
            "self.pipeline.calculate_compliance_rate",
        ),
        (
            "self.pipeline.visualize_hairnet_detections",
            "self.pipeline.visualize_detections",
        ),
...
"""
        ...
        if imports_end:
            content = content.replace(
                imports_end.group(0), imports_end.group(0) + mock_classes
            )
