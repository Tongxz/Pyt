...
def analyze_color_features(roi):
    ...
    if upper_roi.size > 0:
        ...
        upper_light_blue_ratio = np.sum(upper_light_blue_mask > 0) / (upper_roi.shape[0] * upper_roi.shape[1])
...
def analyze_edge_features(roi):
    ...
    if upper_roi.size > 0:
        ...
        upper_edge_density = np.sum(upper_edges > 0) / (upper_roi.shape[0] * upper_roi.shape[1])
    else:
