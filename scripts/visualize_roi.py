...
class ROIVisualizer:
    ...
    def extract_head_roi(self, image: np.ndarray, bbox: List[float], keypoints: Optional[List[float]] = None) -> Tuple[np.ndarray, Dict]:
        """提取头部ROI区域"""
        ...
        roi_info = {
            ...
            'method': 'keypoints' if keypoints and len(keypoints) >= 15 else 'bbox_estimation'
        }
    ...
    def visualize_single_image(self, image_path: str, save_path: Optional[str] = None):
        ...
        for i, detection in enumerate(detections):
            ...
            head_rect = patches.Rectangle((hx1, hy1), hx2-hx1, hy2-hy1,
            ...
            if roi.size > 0:
                ax2.imshow(roi)
                ax2.set_title(f'Head ROI\nSize: {roi_info["size"]}\nMethod: {roi_info["method"]}')
            else:
                ax2.text(0.5, 0.5, 'ROI提取失败', ha='center', va='center', transform=ax2.transAxes)
            ...
            if roi.size > 0:
                ...
                ax3.text(0.5, 0.5, '无ROI数据', ha='center', va='center', transform=ax3.transAxes)
            ...
            if roi.size > 0:
                ...
                ax4.text(0.5, 0.5, '无ROI数据', ha='center', va='center', transform=ax4.transAxes)
    ...
    def visualize_batch(self, image_dir: str, output_dir: str = "roi_visualizations"):
        ...
        for i, image_path in enumerate(image_files):
            ...
            output_name = os.path.splitext(os.path.basename(image_path))[0] + '_roi_analysis.png'
...
def main():
    ...
    parser.add_argument('--output', '-o', type=str, default='roi_visualizations', help='输出目录')
