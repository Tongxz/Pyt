...
def parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="./models/hairnet_detector.pt", help="模型路径"
    )
    parser.add_argument(
        "--source", type=str, required=True, help="测试图像或视频路径，或摄像头索引（0表示默认摄像头）"
    )
    parser.add_argument("--conf", type=float, default=0.25, help="置信度阈值")
    parser.add_argument("--iou", type=float, default=0.45, help="IoU阈值")
    parser.add_argument(
        "--device", type=str, default="auto", help="推理设备，可选：cpu, cuda, auto"
    )
...
def test_model(args):
    ...
    return {"frame_count": frame_count, "detection_count": detection_count}
