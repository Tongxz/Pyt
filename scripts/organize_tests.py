...
def ensure_dirs():
    """确保所有目录存在"""
    for dir_path in [UNIT_TESTS_DIR, INTEGRATION_TESTS_DIR, FIXTURES_DIR, 
                     IMAGES_DIR, VIDEOS_DIR, PERSON_IMAGES_DIR, HAIRNET_IMAGES_DIR]:
...
def is_unit_test(file_path):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        return ("unittest" in content or 
                "pytest" in content or 
                "TestCase" in content or 
                re.search(r"def test_\w+\(.*\):", content) is not None) and \
               not is_integration_test(file_path)
...
def is_person_image(file_path):
    """判断是否为人物图像"""
    return "person" in file_path.name.lower() and not "hairnet" in file_path.name.lower()
