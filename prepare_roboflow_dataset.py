...
def parse_args():
    ...
    parser.add_argument('--input', type=str, required=True, 
                        help='Roboflow数据集的ZIP文件路径或已解压的目录路径')
...
def detect_roboflow_format(input_dir):
    """检测Roboflow数据集的格式"""
    # 检查是否有data.yaml文件
    yaml_files = [f for f in os.listdir(input_dir) 
    ...
    # 检查目录结构
    if os.path.exists(os.path.join(input_dir, 'train')) and \
       os.path.exists(os.path.join(input_dir, 'valid')):
        if os.path.exists(os.path.join(input_dir, 'train', 'images')) and \
           os.path.exists(os.path.join(input_dir, 'train', 'labels')):
            print("检测到YOLO格式数据集目录结构")
            return 'yolo', None
    
    if os.path.exists(os.path.join(input_dir, 'train')) and \
       os.path.exists(os.path.join(input_dir, 'valid')) and \
       os.path.exists(os.path.join(input_dir, '_annotations.coco.json')):
        print("检测到COCO格式数据集")
        return 'coco', None
...
def main():
    ...
    try:
        ...
        if validate_dataset(args.output):
            ...
            print("\n使用以下命令训练YOLOv8模型:")
            print(f"python train_hairnet_model.py --data {yaml_path} --epochs 100 --batch-size 16 --img-size 640")
        else:
