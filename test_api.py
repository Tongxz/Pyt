...
def test_comprehensive_detection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"发网统计: 佩戴={stats.get('persons_with_hairnet', 0)}, 未佩戴={stats.get('persons_without_hairnet', 0)}")
            print(f"行为统计: 洗手={stats.get('persons_handwashing', 0)}, 消毒={stats.get('persons_sanitizing', 0)}")
            print(f"合规率: 发网={stats.get('hairnet_compliance_rate', 0):.2%}, 洗手={stats.get('handwash_rate', 0):.2%}, 消毒={stats.get('sanitize_rate', 0):.2%}")
            
            processing_time = result.get('processing_time', {})
            print(f"处理时间: 总计={processing_time.get('total_time', 0):.3f}秒")
            
            # 检查是否有标注图像
            has_image = 'annotated_image' in result and result['annotated_image'] is not None
