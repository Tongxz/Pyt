...
class DevEnvironmentChecker:
                    ...
                    for line in f:
                        ...
                        if line and not line.startswith('#'):
                            ...
                            if '>=' in line:
                                ...
                                requirements[package.strip()] = version.split(',')[0].strip()
                            elif '==' in line:
                                package, version = line.split('==', 1)
    ...
    def check_dependencies(self) -> Tuple[List[str], List[str], List[str]]:
        ...
        key_packages = [
            ...
            'ultralytics', 'opencv-python', 'fastapi',
            'numpy', 'pillow', 'pyyaml', 'uvicorn'
        ]
                ...
                if required_version != "any":
                    ...
                    try:
                        if self._compare_versions(installed_version, required_version):
                            ...
                            print(f"{status} {package:<20} {installed_version:<15} (>= {required_version})")
                        else:
                            outdated.append(package)
                            status = f"{Colors.YELLOW}⚠{Colors.NC}"
                            print(f"{status} {package:<20} {installed_version:<15} (需要 >= {required_version})")
                    except:
                        satisfied.append(package)
                        print(f"{status} {package:<20} {installed_version:<15} (版本检查失败)")
                else:
                    satisfied.append(package)
