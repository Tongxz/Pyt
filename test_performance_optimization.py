...
def test_performance_comparison():
    ...
    pipeline = OptimizedDetectionPipeline(
        human_detector=human_detector, hairnet_detector=hairnet_detector
    )
    result = pipeline.detect_comprehensive(
        image, enable_handwash=False, enable_sanitize=False
    )
    ...
    persons_with_hairnet = sum(
        1 for r in result.hairnet_results if r.get("has_hairnet", False)
    )
    compliance_rate = (
    ...
    print("=== 性能对比结果 ===")
    improvement = (
        ((method1_time - method2_time) / method1_time * 100) if method1_time > 0 else 0
    )
