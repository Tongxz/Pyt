    ...
    try:
        ...
        for image_path in test_images:
            ...
            try:
                ...
                # 发网检测结果
                hairnet_count = sum(
                    1 for h in result.hairnet_results if h.get("wearing_hairnet", False)
                )
                hairnet_rate = (
                    (hairnet_count / len(result.person_detections) * 100)
                ...
                # 洗手检测结果
                handwash_count = sum(
                    1 for h in result.handwash_results if h.get("is_handwashing", False)
                )
                handwash_rate = (
                    (handwash_count / len(result.person_detections) * 100)
                    if result.person_detections
                    else 0
                )
                ...
                # 消毒检测结果
                sanitize_count = sum(
                    1 for s in result.sanitize_results if s.get("is_sanitizing", False)
                )
                sanitize_rate = (
                    (sanitize_count / len(result.person_detections) * 100)
                    if result.person_detections
                    else 0
                )
                ...
                if result.handwash_results:
                    ...
                    for handwash in result.handwash_results:
                        ...
                        print(
                            f"    人员{person_id}: 洗手={is_handwashing}, 置信度={confidence:.3f}"
                        )
                ...
                if result.sanitize_results:
                    ...
                    for sanitize in result.sanitize_results:
                        ...
                        print(
                            f"    人员{person_id}: 消毒={is_sanitizing}, 置信度={confidence:.3f}"
                        )
