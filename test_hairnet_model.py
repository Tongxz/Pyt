...
def parse_args():
    ...
    parser.add_argument("--iou-thres", type=float, default=0.45, help="IoU阈值，默认为0.45")
    ...
    parser.add_argument("--classes", nargs="+", type=int, help="按类别过滤，例如--classes 0")
    parser.add_argument("--project", type=str, default="runs/detect", help="保存结果的项目目录")
    parser.add_argument("--name", type=str, default="exp", help="保存结果的子目录名称")
    parser.add_argument("--exist-ok", action="store_true", help="如果目录存在则不创建新目录")
    parser.add_argument("--line-thickness", type=int, default=3, help="边界框厚度(像素)")
