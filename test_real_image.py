...
def create_realistic_person_image():
    ...
    # 左腿
    draw.rectangle(
        [center_x - leg_width - 10, leg_top, center_x - 10, leg_top + leg_length],
        fill="darkblue",
        outline="black",
        width=2,
    )
    ...
    # 右腿
    draw.rectangle(
        [center_x + 10, leg_top, center_x + leg_width + 10, leg_top + leg_length],
        fill="darkblue",
        outline="black",
        width=2,
    )
    ...
    # 眼睛
    draw.ellipse(
        [center_x - 25, head_top + 30, center_x - 15, head_top + 40], fill="black"
    )
    draw.ellipse(
        [center_x + 15, head_top + 30, center_x + 25, head_top + 40], fill="black"
    )

    # 嘴巴
    draw.arc(
        [center_x - 15, head_top + 60, center_x + 15, head_top + 80],
        0,
        180,
        fill="black",
        width=2,
...
def test_with_image(image, image_name):
    ...
    # 准备文件数据
    files = {"file": (f"{image_name}.jpg", BytesIO(buffer.tobytes()), "image/jpeg")}

    try:
        # 发送请求到综合检测API
        response = requests.post(
            "http://localhost:8000/api/v1/detect/comprehensive", files=files, timeout=30
        )
...
def create_multi_person_image():
    ...
    for i, (pos, (shirt_color, skin_color)) in enumerate(zip(positions, colors)):
        ...
        # 左腿
        draw.rectangle(
            [center_x - leg_width - 5, leg_top, center_x - 5, leg_top + leg_length],
            fill="darkblue",
            outline="black",
            width=2,
        )
        ...
        # 右腿
        draw.rectangle(
            [center_x + 5, leg_top, center_x + leg_width + 5, leg_top + leg_length],
            fill="darkblue",
            outline="black",
            width=2,
        )
