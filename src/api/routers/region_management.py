...
def get_all_regions(region_service: RegionService = Depends(get_region_service)) -> List[Dict[str, Any]]:
    return region_service.get_all_regions()

@router.post("/regions", summary="创建新区域")
def create_region(region_data: Dict[str, Any], region_service: RegionService = Depends(get_region_service)) -> Dict[str, Any]:
    try:
...
def update_region(region_id: str, region_data: Dict[str, Any], region_service: RegionService = Depends(get_region_service)) -> Dict[str, Any]:
    try:
...
def delete_region(region_id: str, region_service: RegionService = Depends(get_region_service)) -> Dict[str, Any]:
    try:
