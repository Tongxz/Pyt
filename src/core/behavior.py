...
class BehaviorRecognizer:
    ...
    def __init__(self, confidence_threshold: float = 0.6):
        ...
        logger.info(f"BehaviorRecognizer initialized with threshold: {confidence_threshold}")
    
    def detect_hairnet(self, person_bbox: List[int], head_region: Optional[Dict] = None) -> float:
        """
        检测发网佩戴
    ...
    def detect_handwashing(self, person_bbox: List[int], hand_regions: List[Dict]) -> float:
        """
        检测洗手行为
    ...
    def detect_sanitizing(self, person_bbox: List[int], hand_regions: List[Dict]) -> float:
        """
        检测手部消毒行为
        ...
        if len(hand_regions) >= 2:
            ...
            # 计算双手距离
            distance = ((hand1_center[0] - hand2_center[0]) ** 2 + 
                       (hand1_center[1] - hand2_center[1]) ** 2) ** 0.5
    ...
    def update_behavior(self, track_id: int, person_bbox: List[int], 
                       additional_info: Optional[Dict] = None) -> Dict[str, BehaviorState]:
        """
        更新指定追踪目标的行为状态
    ...
    def _update_active_behaviors(self, track_id: int, current_behaviors: Dict[str, BehaviorState]):
        """
        更新活跃行为状态
        ...
        for behavior_type, behavior_state in current_behaviors.items():
            if behavior_type in self.active_behaviors[track_id]:
                # 更新现有行为
                self.active_behaviors[track_id][behavior_type].update(behavior_state.confidence)
            else:
                # 添加新行为
        ...
        for behavior_type, behavior_state in self.active_behaviors[track_id].items():
            if behavior_type not in current_behaviors:
                ...
                if behavior_state.duration >= min_duration:
                    logger.info(f"Behavior {behavior_type} completed for track {track_id}, "
