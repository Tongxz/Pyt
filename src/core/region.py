...
class Region:
    ...
    def __init__(self, region_id: str, region_type: RegionType, 
                 polygon: List[Tuple[int, int]], name: str = ""):
        """
        初始化区域
...
class RegionManager:
    ...
    def _trigger_violation(self, region_id: str, track_id: int, violation_type: str):
        ...
        if region.rules['alert_on_violation']:
            logger.warning(f"Violation in region {region.name}: {violation_type} by track {track_id}")
    ...
    def check_behavior_compliance(self, track_id: int, behaviors: Dict) -> List[Dict]:
        ...
        for region_id in self.track_regions[track_id]:
            ...
            for required_behavior in region.rules['required_behaviors']:
                if required_behavior not in behaviors or not behaviors[required_behavior].is_active:
                    violation = {
                        'region_id': region_id,
            ...
            for forbidden_behavior in region.rules['forbidden_behaviors']:
                if forbidden_behavior in behaviors and behaviors[forbidden_behavior].is_active:
                    violation = {
                        'region_id': region_id,
                        'region_name': region.name,
                        'track_id': track_id,
