...
class OptimizedDetectionPipeline:
    ...
    def detect_comprehensive(self, image: np.ndarray, 
        ...
        self.stats["avg_processing_time"] = (
            (self.stats["avg_processing_time"] * (self.stats["total_detections"] - 1) + total_time) /
    ...
    def _execute_detection_pipeline(self, image: np.ndarray,
                                  enable_hairnet: bool,
                                  enable_handwash: bool,
                                  enable_sanitize: bool) -> DetectionResult:
        """
        ...
        if (enable_handwash or enable_sanitize) and len(person_detections) > 0:
            ...
            if enable_handwash:
                handwash_results = self._detect_handwash_for_persons(image, person_detections)
            
            if enable_sanitize:
                sanitize_results = self._detect_sanitize_for_persons(image, person_detections)
        ...
        annotated_image = self._create_annotated_image(
            image, person_detections, hairnet_results, handwash_results, sanitize_results
        )
    ...
    def _detect_hairnet_for_persons(self, image: np.ndarray, 
        ...
        try:
            ...
            if hasattr(self.hairnet_detector, 'detect_hairnet_compliance'):
                ...
                compliance_result = self.hairnet_detector.detect_hairnet_compliance(image, person_detections)
                ...
                for i, person_detection in enumerate(person_detections):
                    ...
                    if i < len(detections):
                        ...
                        hairnet_confidence = detection_info.get("hairnet_confidence", 0.0)
                ...
                for i, detection in enumerate(person_detections):
                    try:
                        ...
                        if head_y2 > head_y1 and head_x2 > head_x1:
                            ...
                            hairnet_result = self.hairnet_detector.detect_hairnet_compliance(head_region)
    ...
    def _detect_handwash_for_persons(self, image: np.ndarray, 
                                   person_detections: List[Dict]) -> List[Dict]:
        """为检测到的人员进行洗手行为检测"""
        ...
        for i, detection in enumerate(person_detections):
            try:
                ...
                if person_region.size > 0:
                    ...
                    confidence = self.behavior_recognizer.detect_handwashing(bbox, hand_regions)
                    is_handwashing = confidence > self.behavior_recognizer.confidence_threshold
                else:
    ...
    def _detect_sanitize_for_persons(self, image: np.ndarray, 
                                   person_detections: List[Dict]) -> List[Dict]:
        """为检测到的人员进行消毒行为检测"""
        if self.behavior_recognizer is None:
            logger.warning("行为识别器未初始化，使用模拟结果")
        ...
        for i, detection in enumerate(person_detections):
            try:
                ...
                if person_region.size > 0:
                    ...
                    # 需要提供手部区域信息
                    hand_regions = self._estimate_hand_regions(bbox)
                    confidence = self.behavior_recognizer.detect_sanitizing(bbox, hand_regions)
                    is_sanitizing = confidence > self.behavior_recognizer.confidence_threshold
                else:
                    is_sanitizing = False
                    confidence = 0.0
    ...
    def _create_annotated_image(self, image: np.ndarray,
                              ...
                              hairnet_results: List[Dict],
                              handwash_results: List[Dict],
                              sanitize_results: List[Dict]) -> np.ndarray:
        """创建带注释的结果图像"""
    ...
    def update_models(self, human_detector=None, hairnet_detector=None, behavior_recognizer=None):
        """更新模型（热更新支持）"""
...
class VideoStreamOptimizer:
    ...
    def __init__(self, detection_pipeline: OptimizedDetectionPipeline,
                 frame_skip: int = 3,  # 每3帧处理一次
                 similarity_threshold: float = 0.95):  # 帧相似度阈值
    ...
    def process_frame(self, frame: np.ndarray, force_process: bool = False) -> Optional[DetectionResult]:
        """处理视频帧（带优化）"""
        self.frame_count += 1
        ...
        if not force_process and self.last_processed_frame is not None:
            similarity = self._calculate_frame_similarity(frame, self.last_processed_frame)
