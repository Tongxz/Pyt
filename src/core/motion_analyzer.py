...
class MotionAnalyzer:
    ...
    def __init__(self):
        ...
        初始化运动分析器
        """
        self.hand_trackers = {}  # track_id -> {'left': MotionTracker, 'right': MotionTracker}
        
        # 洗手行为特征阈值
        self.handwash_thresholds = {
            'min_movement_ratio': 1.5,  # 水平运动/垂直运动比例
            'min_avg_speed': 0.01,      # 最小平均速度
            'max_avg_speed': 0.5,       # 最大平均速度
            'min_position_variance': 0.001,  # 最小位置方差
    ...
    def analyze_sanitizing(self, track_id: int) -> float:
        ...
        if motion_confidences:
            ...
            # 根据双手距离调整置信度
            distance_factor = 1.0 - (distance / self.sanitize_thresholds['max_hand_distance'])
    ...
    def _evaluate_handwash_motion(self, motion_stats: Dict) -> float:
        ...
        # 检查运动比例（水平运动应该更多）
        if motion_stats['movement_ratio'] >= self.handwash_thresholds['min_movement_ratio']:
            confidence += 0.3
        
        # 检查平均速度
        avg_speed = motion_stats['avg_speed']
        if (self.handwash_thresholds['min_avg_speed'] <= avg_speed <= 
            self.handwash_thresholds['max_avg_speed']):
            confidence += 0.3
        
        # 检查位置方差（应该有一定的运动范围）
        if motion_stats['position_variance'] >= self.handwash_thresholds['min_position_variance']:
            confidence += 0.2
    ...
    def _evaluate_sanitize_motion(self, motion_stats: Dict) -> float:
        ...
        # 检查运动比例（消毒时运动比例较小）
        if motion_stats['movement_ratio'] >= self.sanitize_thresholds['min_movement_ratio']:
            confidence += 0.3
        
        # 检查平均速度
        avg_speed = motion_stats['avg_speed']
        if (self.sanitize_thresholds['min_avg_speed'] <= avg_speed <= 
