...
class DetectionDataManager:
    ...
    def save_detection_result(self, 
        ...
        try:
            ...
            persons_with_hairnet = sum(1 for r in detection_results if r.get('has_hairnet', False))
            
            # 计算平均置信度
            if detection_results:
                avg_confidence = sum(r.get('hairnet_confidence', 0) for r in detection_results) / total_persons
            else:
                avg_confidence = 0.0
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.debug(f"保存检测结果: frame_id={frame_id}, persons={total_persons}, with_hairnet={persons_with_hairnet}")
                return True
    ...
    def export_data(self, output_path: str, start_date: Optional[str] = None, end_date: Optional[str] = None) -> bool:
        """
        导出数据到CSV文件
