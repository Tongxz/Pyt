...
class HairnetDetector:
    ...
    def detect_hairnet(self, frame: np.ndarray, human_bbox: Optional[List[int]] = None, 
        ...
        try: 
            ...
            if self.use_enhanced_roi:
                roi_enhanced = self._extract_head_roi_enhanced(frame, human_bbox, keypoints)
                if roi_enhanced is not None:
                    self.logger.info(f"增强ROI提取成功，方法: {roi_enhanced.get('strategy', 'unknown')}，质量评分: {roi_enhanced.get('quality_score', 0):.3f}")
            
            # 备选：关键点ROI 
            if roi_enhanced is None and keypoints is not None and self.pose_detector is not None: 
                roi_kp = self._optimize_head_roi_with_keypoints(frame, human_bbox, keypoints) 
            ...
            # 添加调试信息 
            self.logger.info(f"发网检测结果: 佩戴={hairnet_result['wearing_hairnet']}, " 
    ...
    def _optimize_head_roi_with_keypoints(self, image: np.ndarray, person_bbox: Optional[List[int]], keypoints: np.ndarray) -> Optional[Dict[str, Any]]:
        """
        使用关键点优化头部ROI提取
    ...
    def _extract_head_roi_enhanced(self, image: np.ndarray, person_bbox: Optional[List[int]], keypoints: Optional[np.ndarray] = None) -> Optional[Dict[str, Any]]:
        """
        使用增强算法提取头部ROI
        ...
        try:
            ...
            # 使用增强的多方法ROI提取
            head_roi, roi_info = self.enhanced_roi_extractor.extract_head_roi_multi_method(
    ...
    def _extract_head_roi_from_bbox(self, image: np.ndarray, person_bbox: Optional[List[int]]) -> Optional[Dict[str, Any]]:
        """
        从人体边界框提取头部ROI（备份方法）
    ...
    def _detect_hairnet_with_cnn(self, head_region: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算颜色比例
            light_blue_ratio = np.sum(light_blue_mask > 0) / (light_blue_mask.shape[0] * light_blue_mask.shape[1])
            light_color_ratio = np.sum(light_mask > 0) / (light_mask.shape[0] * light_mask.shape[1])
            white_ratio = np.sum(white_mask > 0) / (white_mask.shape[0] * white_mask.shape[1])
            ...
            result = {
                ...
                'hairnet_color': self._determine_hairnet_color(float(light_blue_ratio), float(light_color_ratio), float(white_ratio), float(green_ratio)),
                'hairnet_pixels': int(np.sum(light_blue_mask > 0) + np.sum(light_mask > 0) + np.sum(white_mask > 0) + np.sum(green_mask > 0)),
                ...
                'debug_info': {
                    ...
                    'model_output': outputs.cpu().numpy().tolist()[0] if isinstance(outputs, torch.Tensor) else None,
                    'probabilities': probabilities.cpu().numpy().tolist()[0] if isinstance(probabilities, torch.Tensor) else None
                }
            }
    ...
    def detect_hairnet_with_keypoints(self, frame: np.ndarray, human_bbox: Optional[List[int]] = None, 
                       keypoints: Optional[np.ndarray] = None) -> Dict[str, Any]: 
        """检测发网佩戴状态（支持传入关键点）""" 
        try: 
            # 检查依赖注入的检测器 
            ...
            # 添加调试信息 
            self.logger.info(f"发网检测结果: 佩戴={hairnet_result.get('has_hairnet', False)}, " 
    ...
    def _analyze_hairnet_color(self, head_region: np.ndarray, mask: Optional[np.ndarray] = None) -> Dict[str, Any]:
        """
        分析发网颜色
    ...
    def _detect_hairnet_with_pytorch(self, head_region: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            sensitive_edge_density = np.sum(edges_sensitive > 0) / (edges_sensitive.shape[0] * edges_sensitive.shape[1])
            ...
            # 分析轮廓特征
            small_contours = [c for c in contours if cv2.contourArea(c) < 100 and cv2.contourArea(c) > 10]
            ...
            upper_edge_density = np.sum(upper_half > 0) / (upper_half.shape[0] * upper_half.shape[1])
            
            # 上部区域的颜色特征
            upper_light_blue = light_blue_mask[:light_blue_mask.shape[0]//2, :]
            upper_light_blue_ratio = np.sum(upper_light_blue > 0) / (upper_light_blue.shape[0] * upper_light_blue.shape[1])
            ...
            # 综合得分（包含颜色特征）
            total_score = edge_score + contour_score + upper_score + light_blue_score + light_color_score + upper_blue_score
            
            # 智能检测阈值 - 平衡调整，减少误检但保持检测能力
            # 情况1：明显的浅蓝色发网特征（降低要求）
            has_light_blue_hairnet = (
                light_blue_ratio > 0.004 and  # 降低浅蓝色区域要求
                upper_light_blue_ratio > 0.003 and  # 降低上部浅蓝色要求
                edge_density > 0.015 and  # 降低边缘特征要求
                len(small_contours) > 3 and  # 降低轮廓要求
                upper_edge_density > 0.018  # 降低上部边缘密度要求
            )
            
            # 情况2：一般的发网检测（降低要求）
            has_general_hairnet = (
                total_score > 1.2 and  # 降低综合得分要求
                edge_density > 0.015 and  # 降低边缘密度要求
                len(small_contours) > 3 and  # 降低轮廓数量要求
                upper_edge_density > 0.018 and  # 降低上部区域边缘密度要求
                total_color_ratio > 0.006  # 降低颜色比例要求
            )
            
            # 情况3：浅色发网的特殊检测（降低要求）
            has_light_hairnet = (
                (light_blue_ratio > 0.003 or light_color_ratio > 0.006 or white_ratio > 0.005 or green_ratio > 0.003) and  # 降低颜色特征要求
                edge_density > 0.012 and  # 降低边缘要求
                len(small_contours) > 3 and  # 降低轮廓要求
                upper_edge_density > 0.015 and  # 降低上部边缘要求
                total_color_ratio > 0.004  # 降低总颜色比例要求
            )
            ...
            has_basic_hairnet = (
                ...
                len(small_contours) > 3 and  # 降低轮廓要求
                upper_edge_density > 0.015 and  # 降低上部边缘要求
                total_color_ratio > 0.004  # 降低综合颜色比例要求
            )
            
            # 情况5：最低检测标准（降低要求以提高检测率）
            has_minimal_hairnet = (
                (edge_density > 0.010 and len(small_contours) > 2 and total_color_ratio > 0.004 and upper_edge_density > 0.012) or  # 降低基本要求
                (total_color_ratio > 0.006 and upper_edge_density > 0.015 and len(small_contours) > 2 and edge_density > 0.010) or  # 降低综合要求
                (light_color_ratio > 0.005 and edge_density > 0.010 and upper_edge_density > 0.012 and len(small_contours) > 2) or  # 降低浅色要求
                (upper_edge_density > 0.018 and total_score > 1.0 and len(small_contours) > 3 and total_color_ratio > 0.004)  # 降低综合要求
            )
            
            # 最终判断：满足任一条件即可
            has_hairnet = (
                has_light_blue_hairnet or 
                has_general_hairnet or 
                has_light_hairnet or
                has_basic_hairnet or
                has_minimal_hairnet
            )
            ...
            if has_hairnet:
                # 基于多个特征的置信度计算（包含颜色特征）
                base_confidence = (
                    min(0.25, edge_density * 1.5) +  # 边缘密度贡献
                    min(0.2, len(small_contours) * 0.01) +  # 轮廓数量贡献
                    min(0.2, upper_edge_density * 1.2)  # 上部区域贡献
                )
                
                # 颜色特征置信度加成
                color_confidence = (
                    min(0.15, light_blue_ratio * 7.5) +  # 浅蓝色贡献
                    min(0.1, light_color_ratio * 3.3) +  # 浅色贡献
                    min(0.1, upper_light_blue_ratio * 10)  # 上部浅蓝色贡献
                )
    ...
    def _determine_hairnet_color(self, light_blue_ratio: float, light_color_ratio: float, 
                                white_ratio: float, green_ratio: float) -> str:
        """根据颜色比例确定发网颜色"""
        color_ratios = {
    ...
    def detect_hairnet_compliance(self, image):
        ...
        try:
            ...
            for person in persons:
                ...
                if hasattr(self.hairnet_detector, 'use_enhanced_roi') and self.hairnet_detector.use_enhanced_roi:
                    # 尝试使用增强ROI提取
                ...
                if head_roi_result is None:
                    ...
                    head_height = int(person_height * 0.3)
                    head_region = image[y1:y1+head_height, x1:x2]
                    head_roi_result = {'roi': head_region, 'coords': [x1, y1, x2, y1+head_height]} if head_region.size > 0 else None
                ...
                # 发网检测 - 优先使用CNN模型
                try:
                    hairnet_result = self.hairnet_detector._detect_hairnet_with_cnn(head_region)
                except Exception as e:
                    logger.warning(f"CNN模型检测失败，回退到传统方法: {e}")
                    hairnet_result = self.hairnet_detector._detect_hairnet_with_pytorch(head_region)
            ...
            if results['total_persons'] > 0:
                results['compliance_rate'] = results['persons_with_hairnet'] / results['total_persons']
                results['average_confidence'] = total_confidence / results['total_persons']
        ...
        for i, person in enumerate(persons):
            try:
                ...
                result = {
                    ...
                    'contour_count': hairnet_result.get('contour_count', 0),
                    'debug_info': hairnet_result.get('debug_info', {}),
                    'class_name': 'person_with_hairnet' if hairnet_result['has_hairnet'] else 'person_without_hairnet'
                }
