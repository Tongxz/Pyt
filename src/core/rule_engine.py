...
class RuleEngine:
    ...
    def _evaluate_single_rule(self, rule: Rule, context: Dict[str, Any]) -> Optional[Violation]:
        """评估单个规则"""
    ...
    def _check_behavior_required(self, rule: Rule, context: Dict[str, Any]) -> Optional[Violation]:
        """检查必需行为规则"""
        required_behaviors = rule.metadata.get('required_behaviors', [])
        current_behaviors = context.get('behaviors', {})
        
        for behavior in required_behaviors:
            if behavior not in current_behaviors or not current_behaviors[behavior].get('is_active', False):
                return Violation(
    ...
    def _check_behavior_forbidden(self, rule: Rule, context: Dict[str, Any]) -> Optional[Violation]:
        """检查禁止行为规则"""
        forbidden_behaviors = rule.metadata.get('forbidden_behaviors', [])
        current_behaviors = context.get('behaviors', {})
        
        for behavior in forbidden_behaviors:
            if behavior in current_behaviors and current_behaviors[behavior].get('is_active', False):
                return Violation(
                    violation_id=f"{rule.rule_id}_{int(time.time() * 1000)}",
                    rule_id=rule.rule_id,
                    rule_name=rule.name,
    ...
    def _check_occupancy_limit(self, rule: Rule, context: Dict[str, Any]) -> Optional[Violation]:
        """检查人数限制规则"""
        ...
        if max_occupancy > 0 and current_occupancy > max_occupancy:
            return Violation(
                ...
                details={'current_occupancy': current_occupancy, 'max_occupancy': max_occupancy}
            )
        
        return None
    
    def _check_duration_limit(self, rule: Rule, context: Dict[str, Any]) -> Optional[Violation]:
        """检查时长限制规则"""
        ...
        if max_duration > 0 and current_duration > max_duration:
            return Violation(
                ...
                details={'current_duration': current_duration, 'max_duration': max_duration}
            )
        
        return None
    
    def _check_time_restriction(self, rule: Rule, context: Dict[str, Any]) -> Optional[Violation]:
        """检查时间限制规则"""
    ...
    def _check_custom_rule(self, rule: Rule, context: Dict[str, Any]) -> Optional[Violation]:
        """检查自定义规则"""
        # 评估所有条件
    ...
    def resolve_violation(self, violation_id: str) -> bool:
        ...
        self.stats['resolved_violations'] = sum(1 for v in self.violations.values() if v.resolved)
    ...
    def _update_stats(self):
        ...
        self.stats['active_rules'] = sum(1 for rule in self.rules.values() if rule.is_active)
        ...
        try:
            ...
            for rule_data in config.get('rules', []):
                rule = Rule(
                    ...
                    conditions=[RuleCondition(**cond) for cond in rule_data.get('conditions', [])],
