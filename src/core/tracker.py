...
class MultiObjectTracker:
    ...
    def __init__(self, max_disappeared: int = 10, iou_threshold: float = 0.3):
        ...
        logger.info(
            f"MultiObjectTracker initialized with IoU threshold: {iou_threshold}"
        )
    ...
    def _match_tracks_detections(
        self, iou_matrix: np.ndarray, track_ids: List[int], detections: List[Dict]
        ...
        # 记录原始索引映射
        available_track_indices = list(range(len(track_ids)))
        available_det_indices = list(range(len(detections)))
        ...
        while True:
            ...
            matrix_track_idx, matrix_det_idx = max_iou_idx

            # 获取原始索引
            original_track_idx = available_track_indices[matrix_track_idx]
            original_det_idx = available_det_indices[matrix_det_idx]

            matched_tracks.append(original_track_idx)
            matched_detections.append(original_det_idx)

            # 移除已匹配的行和列
            iou_matrix = np.delete(iou_matrix, matrix_track_idx, axis=0)
            iou_matrix = np.delete(iou_matrix, matrix_det_idx, axis=1)

            # 更新可用索引列表
            available_track_indices.pop(matrix_track_idx)
            available_det_indices.pop(matrix_det_idx)
