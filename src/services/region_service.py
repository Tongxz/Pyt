...
from core.region import RegionManager, Region, RegionType
...
def get_region_service(region_manager: RegionManager = Depends(get_region_manager)) -> "RegionService":
...
def initialize_region_service(regions_file: str):
    ...
    try:
        ...
        if not region_manager.load_regions_config(regions_file):
             logger.warning(f"Could not load regions from {regions_file}. Starting with an empty set.")
