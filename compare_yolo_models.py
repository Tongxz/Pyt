...
"""
YOLO模型对比测试脚本
比较yolov8n.pt和yolov8m.pt的人体检测效果
"""
...
def test_model_comparison():
    ...
    # 测试模型列表
    models_to_test = [
        {"name": "YOLOv8n (Nano)", "path": "yolov8n.pt", "description": "最小模型，速度最快"},
        {"name": "YOLOv8s (Small)", "path": "yolov8s.pt", "description": "小型模型，平衡速度和精度"},
        {"name": "YOLOv8m (Medium)", "path": "yolov8m.pt", "description": "中型模型，更高精度"},
        {"name": "YOLOv8l (Large)", "path": "yolov8l.pt", "description": "大型模型，高精度"}
    ]
    ...
    for conf in [0.1, 0.25, 0.5]:
        ...
        for result in results:
            if result['conf_threshold'] == conf:
                print(f"{result['model']:<15} {result['person_count']:<8} {result['high_conf_count']:<10} "
    ...
    if conf_025_results:
        # 按检测人数排序
        by_detection = sorted(conf_025_results, key=lambda x: x['person_count'], reverse=True)
        # 按平均置信度排序
        by_confidence = sorted(conf_025_results, key=lambda x: x['avg_confidence'], reverse=True)
        # 按速度排序
        by_speed = sorted(conf_025_results, key=lambda x: x['inference_time'])
        
        print(f"检测人数最多: {by_detection[0]['model']} ({by_detection[0]['person_count']}人)")
        print(f"平均置信度最高: {by_confidence[0]['model']} ({by_confidence[0]['avg_confidence']:.3f})")
        ...
        for result in conf_025_results:
            detection_score = result['person_count'] / max([r['person_count'] for r in conf_025_results]) * 0.4
            confidence_score = result['avg_confidence'] * 0.4
            speed_score = (1 / result['inference_time']) / max([1/r['inference_time'] for r in conf_025_results]) * 0.2
