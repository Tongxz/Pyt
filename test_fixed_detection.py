...
def create_test_image_with_people():
    ...
    for i, (x, y) in enumerate(people_positions):
        ...
        # 绘制手臂
        draw.rectangle(
            [x - 45, y + 20, x - 25, y + 80], fill="lightgreen", outline="green"
        )  # 左臂
        draw.rectangle(
            [x + 25, y + 20, x + 45, y + 80], fill="lightgreen", outline="green"
        )  # 右臂

        # 绘制腿部
        draw.rectangle(
            [x - 20, y + 120, x - 5, y + 200], fill="lightgreen", outline="green"
        )  # 左腿
        draw.rectangle(
            [x + 5, y + 120, x + 20, y + 200], fill="lightgreen", outline="green"
        )  # 右腿
...
def test_detection_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 检查各项检测是否有结果
            hairnet_count = statistics.get("persons_with_hairnet", 0) + statistics.get(
