            ...
            try:
                ...
                for i, detection in enumerate(person_detections):
                    ...
                    if head_y2 > head_y1 and head_x2 > head_x1:
                        ...
                        # 保存头部区域图像用于调试
                        head_filename = f"head_region_person_{i+1}_{Path(image_path).stem}.jpg"
                        cv2.imwrite(head_filename, head_region)
                        print(f"    头部区域已保存: {head_filename}")
                        
                        # 发网检测
                        print("    执行发网检测...")
                        hairnet_result = hairnet_detector.detect_hairnet_compliance(head_region)
                        
                        print(f"    发网检测结果:")
                        print(f"      佩戴发网: {hairnet_result.get('wearing_hairnet', False)}")
                        print(f"      置信度: {hairnet_result.get('confidence', 0):.3f}")
                        print(f"      头部ROI坐标: {hairnet_result.get('head_roi_coords', [])}")
                    else:
                        print("    头部区域无效，跳过发网检测")
